...
public class SubscriptionListManager {
    ...
    @Transactional
    public void removeSubscriptionFromList(FinanceTarget financeTarget) {
        subscriptionRepository.findAllByCustomerEmail(financeTarget.getCustomerEmail())
                .ifPresentOrElse(
                        subs -> {
                            subs.getSubscriptions().removeIf(detail -> detail.getId().equals(
                                    financeTarget.getId()));
                        ...
                        () -> {
                            throw new SubscriptionDeletionException(
                                    "Subscription for customer email %s not found.".formatted(
                                            financeTarget.getCustomerEmail()));
                        }
                );
    }
    ...
    public List<SubscriptionDetailDTO> findAllCustomerSubscriptions(String email) {
        return subscriptionRepository.findAllByCustomerEmail(email)
                .stream()
                .flatMap(s -> s.getSubscriptions()
                        .stream()
                        .map(subscriptionDetailDTOMapper::mapToDTO))
